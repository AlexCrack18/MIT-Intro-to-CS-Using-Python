...
def get_frequencies(input_iterable):
    ...
    """
    temp_word_dict = {}
    for item in set(input_iterable):
        temp_word_dict[item] = input_iterable.count(item)
...
def get_letter_frequencies(word):
    ...
    for letter in set(word):
        temp_letter_dict[letter] = word.count(letter)
    return temp_letter_dict
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    """
    u = set(freq_dict1).union(set(freq_dict2))
    freq_differences = 0
    freq_totals = 0
    for word in u:
        freq_differences += abs(freq_dict1.get(word, 0) - freq_dict2.get(word, 0))
        freq_totals += freq_dict2.get(word, 0) + freq_dict1.get(word, 0)
    if freq_totals == 0:
        return 1
    return round(1 - (freq_differences/freq_totals), 2)
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for word in u:
        num_reps = 0
        num_reps += freq_dict1.get(word, 0) + freq_dict2.get(word, 0)
        if num_reps > max_num_reps:
            max_num_reps = num_reps
            most_freq_word.clear()
            most_freq_word.append(word)
        elif num_reps == max_num_reps:
            most_freq_word.append(word)    
    most_freq_word.sort()
